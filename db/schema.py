...
-- ═══════════════════════════════════════════════════════════════════
-- Anonymous digital-product checkout and append-only event trail
-- ═══════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS digital_product_orders (
    ...
    CONSTRAINT digital_product_orders_public_id_key UNIQUE (public_id),
    CONSTRAINT digital_product_orders_expected_amount_check CHECK (expected_amount > 0),
    CONSTRAINT digital_product_orders_currency_check CHECK (currency = 'VND'),
    CONSTRAINT digital_product_orders_payos_order_code_key UNIQUE (payos_order_code),
    CONSTRAINT digital_product_orders_payment_link_id_key UNIQUE (payment_link_id),
    CONSTRAINT digital_product_orders_status_check
        CHECK (status IN ('pending', 'paid', 'expired', 'cancelled', 'payment_review')),
    CONSTRAINT digital_product_orders_download_count_check CHECK (download_count >= 0)
);
...
CREATE TABLE IF NOT EXISTS digital_product_order_events (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT digital_product_order_events_order_id_fkey
        FOREIGN KEY (order_id) REFERENCES digital_product_orders(id),
    CONSTRAINT digital_product_order_events_order_event_reference_key
...
def _migrate_digital_product_order_schema(conn: Any) -> None:
    """Complete partially created commerce tables using PostgreSQL-safe DDL."""
    order_columns = (
        "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY",
        "public_id TEXT NOT NULL",
        "product_slug TEXT NOT NULL",
    ...
    for table_name, columns in (
        ("digital_product_orders", order_columns),
        ("digital_product_order_events", event_columns),
    ):
    ...
    constraints = (
        ...
        (
            "digital_product_orders",
            "digital_product_orders_public_id_key",
            "UNIQUE (public_id)",
        ),
        (
            "digital_product_orders",
