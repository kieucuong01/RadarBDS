...
def is_actionable_signal(row) -> bool:
    if not bool(_row_value(row, "is_signal", False)):
        return False
    if bool(_row_value(row, "source_quality_recheck", False)):
        return False
    return not (split_quality_flags(_row_value(row, "source_quality_flags", "")) & ACTIONABLE_SUPPRESS_FLAGS)
...
def actionable_signal_sql(alias: str = "v") -> str:
    ...
    parts = [
        ...
        f"COALESCE({alias}.source_quality_recheck,0)=0",
    ]
