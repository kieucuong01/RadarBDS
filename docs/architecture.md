...
- `services/public_prewarm.py` reads at most 20 allowlisted relative routes, never sends Cookie/Authorization, caps response reads at 2 MiB, and reports status only. Configured `/api/listings` warming requires both read-model flags and a positive committed/durable `listings` version, so publication and standalone prewarm cannot launch the legacy 50-second path. `services/public_data_publish.py` passes the just-committed version, mirrors committed versions first, then prewarms only while the shared-cache flag is enabled; Redis/HTTP failures never relabel committed DB data as failed.
...
anonymous GET /, /api/signals, /api/listings, /api/counts, /api/dashboard
  -> Nginx exact-location guest cache (15 s, lock + background update, stale-on-error;
     including the production `/api/listings` location installed on 2026-08-02)
...
This topology collapses repeated/common public keys; it does not turn one 2-vCPU origin into a safe 5,000-unique-cold-query service or high-availability deployment. With Cloudflare as the active shield, definitive run `30762453173` passed the prewarmed 50-key mixed-filter contract through 1,000 concurrent VUs and the common-key homepage contract through 5,000 concurrent VUs. The 5,000 stage handled 1,660,770 requests at conservative max-shard p95/p99 141.255/397.478 ms with 0.0909% failures; all 5,000 first VU iterations started within 183 ms. See `docs/operations.md` for complete stage evidence, observer limits, scope boundaries, and rollback.

Historical distributed run `30698414443` isolated the old direct-origin network path, and run `30756753673` first proved the CDN-required 100-VU stages. A later default-500 attempt was correctly rejected when an unrelated authenticated Safari session exercised the legacy all-listings Maps query. The Maps read-model fix removed that request-time valuation CTE, and run `30762453173` then passed the complete CDN-required serial chain with a clean overlapping observer. Preserve the old failures as diagnostic history, but use the definitive run—not the earlier partial runs—for the current capacity boundary.
...
- Phases 1-4 are deployed: read-model SQL, bounded pool/shared cache, signal-first browser flow, and reversible Redis/Nginx/Gunicorn capacity configuration. Distributed load tooling and CDN-required evidence gates are implemented. The defined guest common-key 5,000-VU and mixed 50-key 1,000-VU acceptance gates passed in run `30762453173`; authenticated traffic remains private and outside that public-cache capacity claim.
...
- `mode=signals` summary and item requests reuse `signal_card_read_model` and the canonical read-model filter predicate, then join `listing_map_locations`. Their cache version combines `public_dataset_versions.signals` with `listing_map_locations.updated_at`; they must not scan valuation history or compute the legacy publisher policy at click time.
- `mode=all` uses the same all-listings predicate over `signal_card_read_model` when both read-model flags are enabled and the durable `listings` version is positive. It does not materialize valuation history at click time. `listing_is_signal` preserves the legacy badge meaning, `complete_only` stays separate from the actionable-signal predicate, and the legacy listing/valuation SQL remains the data-safe flag/version fallback.
- Map cache invalidation combines the applicable durable dataset version with `listing_map_locations.updated_at`. Summary and item loads use a per-process, per-key single-flight lock, so concurrent clicks for the same tier/mode/filter/location produce one heavy query per Gunicorn worker instead of one per request. Waiters release their short version-check connection before entering the lock; only the winning loader acquires a connection for the grouped map query.
