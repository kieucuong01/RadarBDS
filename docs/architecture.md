...
This topology collapses repeated/common public keys; it does not turn one 2-vCPU origin into a safe 5,000-unique-cold-query service or high-availability deployment. The 2026-08-01 production test passed the browser-compressed normal homepage at 100 VUs and a prewarmed 50-key mixed-filter corpus at 500 VUs. External 500 normal and 1,000 mixed stages missed their latency/error gates while Redis, PostgreSQL, CPU, memory, and accept queues remained bounded. Cloudflare is now the active shield, but the distributed serial gates are still required before claiming the 1,000-5,000 target. See `docs/operations.md` for the exact evidence and rollback.

The historical distributed run `30698414443` isolated the old direct-origin network path: all responses/checks were valid while default-100 p95/p99 reached 2.24/3.10 seconds and the origin remained far below abort thresholds. Cloudflare became the active public edge on 2026-08-02. Run `30756753673` then passed CDN-required default-100 and mixed-100 at p95/p99 18.17/34.15 ms and 13.32/45.28 ms. The default-500 stage is not a capacity result: the host observer aborted before k6 started because an unrelated authenticated Safari session exercised the legacy all-listings Maps query and saturated PostgreSQL. Higher stages were skipped. Keep the 5,000 claim open until the Maps fix is deployed and the serial gates are rerun with a clean observer.
