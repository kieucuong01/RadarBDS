...
The mixed setup keeps exactly 50 canonical filter combinations and exercises signals, counts, and listings for each combination; it does not create a cross product or per-VU keys. The second probe for all three routes must be `HIT`. The fixed corpus has a five-minute `setupTimeout`; distributed default shards get a shared VU epoch one minute after synchronized k6 launch so 1,000 VUs per runner can initialize, while mixed shards get four minutes for initialization plus prewarm. Every shard waits for its shared epoch and fails if it misses by more than ten seconds. The aggregator also requires one `radar_vu_started_at_ms` sample per configured VU and rejects late or mismatched starts. A setup/synchronization failure produces no VU capacity result and must not be reported as a pass or failure for that stage. Never run the two scenarios or two stages in parallel. Stop at the first abort threshold from `docs/operations.md`; do not compensate by raising workers, PostgreSQL connections, timeouts, or Redis memory.

The script explicitly requests `Accept-Encoding: gzip` to represent a browser. Treat an uncompressed result as an invalid harness run, not production capacity. `docs/operations.md` preserves the failed/partial history and the authoritative current result: CDN-required run `30762453173` passed every serial gate through common-key `default-5000`, while the 50-key mixed contract stops at `mixed-1000`. Do not broaden that result to unique cold keys, 5,000 RPS, authenticated traffic, origin-only capacity, or high availability.
